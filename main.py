...
class AdminPage:
    ...
    def add_products(self):
        ...
        add_img_label = Label(add_products_page, image=self.add_img)
        add_img_label.place(anchor="c", relx=0.5, rely=0.25)
        add_img_btn = Button(
            add_products_page, text="Select Image", command=self.get_file
        )
        add_img_btn.place(anchor="c", relx=0.5, rely=0.35)
        img_display_label = Label(add_products_page, text="Click on image to change")

        product_name_label = Label(add_products_page, text="Product name")
        product_name_label.place(anchor="c", relx=0.5, rely=0.4)
        product_name_var = StringVar()
        product_name_entry = Entry(add_products_page, textvariable=product_name_var)
        product_name_entry.place(anchor="c", relx=0.5, rely=0.45)

        product_quantity_label = Label(add_products_page, text="Product quantity")
        product_quantity_label.place(anchor="c", relx=0.5, rely=0.5)
        product_quantity_var = IntVar()
        product_quantity_entry = Entry(
            add_products_page, textvariable=product_quantity_var
        )
        product_quantity_entry.place(anchor="c", relx=0.5, rely=0.55)

        product_price_label = Label(add_products_page, text="Product price")
        product_price_label.place(anchor="c", relx=0.5, rely=0.6)
        product_price_var = DoubleVar()
        product_price_entry = Entry(add_products_page, textvariable=product_price_var)
        ...
        def save_product():
            ...
            if resp["status"] == "Success":
                ...
                yes_no = messagebox.askyesnocancel(
                    "Add product", "Do you want to add another item?"
                )
                ...
                if yes_no:
                    ...
                    self.add_img = ImageTk.PhotoImage(
                        Image.open(self.const_url).resize((200, 100))
                    )
                    add_img_btn["image"] = self.add_img
        ...
        save_product_btn = Button(
            add_products_page, text="Save Product", command=save_product
        )
    ...
    def add_users(self):
        ...
        top_label = Label(
            add_users_page, text="ADD NEW USER", fg="blue", font="Times 20 bold"
        )
        top_label.place(anchor="c", relx=0.5, rely=0.15)

        username_label = Label(add_users_page, text="Username", font="Times 15 bold")
        username_label.place(anchor="c", relx=0.5, rely=0.25)
        username_var = StringVar()
        username_entry = Entry(add_users_page, textvariable=username_var)
        username_entry.place(anchor="c", relx=0.5, rely=0.3)

        password_label = Label(add_users_page, text="Password", font="Times 15 bold")
        password_label.place(anchor="c", relx=0.5, rely=0.4)
        password_var = StringVar()
        password_entry = Entry(add_users_page, textvariable=password_var)
        password_entry.place(anchor="c", relx=0.5, rely=0.45)

        message_label = Label(add_users_page, text="Dont have a password in mind? ")
        ...
        generate_password_btn = Button(
            add_users_page, text="Generate Random Password", command=insert_password
        )
        generate_password_btn.place(anchor="c", relx=0.55, rely=0.5)

        def save_user():
            user = (username_var.get(), password_var.get())
        ...
        save_user_btn = Button(add_users_page, text="Save User", command=save_user)
    ...
    def update_user(self, username, user_id):
        ...
        username_label = Label(update_users_page, text="Username", font="Times 15 bold")
        username_label.place(anchor="c", relx=0.5, rely=0.25)
        username_var = StringVar()
        username_var.set(username)
        username_entry = Entry(update_users_page, textvariable=username_var)
        username_entry.place(anchor="c", relx=0.5, rely=0.3)

        password_label = Label(update_users_page, text="Password", font="Times 15 bold")
        password_label.place(anchor="c", relx=0.5, rely=0.4)
        password_var = StringVar()
        password_entry = Entry(update_users_page, textvariable=password_var)
        password_entry.place(anchor="c", relx=0.5, rely=0.45)

        message_label = Label(update_users_page, text="Dont have a password in mind? ")
        message_label.place(anchor="c", relx=0.43, rely=0.5)
        ...
        generate_password_btn = Button(
            update_users_page, text="Generate Random Password", command=insert_password
        )
        generate_password_btn.place(anchor="c", relx=0.55, rely=0.5)

        def update_user():
            user = (username_var.get(), password_var.get(), user_id)
        ...
        save_user_btn = Button(
            update_users_page, text="Update User", command=update_user
        )
        save_user_btn.place(anchor="c", relx=0.5, rely=0.6)
    ...
    def update_product(self, data):
        ...
        update_img_label = Label(
            update_products_page, image=self.images[data.get("img_id") - 1]
        )
        update_img_label.place(anchor="c", relx=0.5, rely=0.25)
        update_img_btn = Button(
            update_products_page, text="Select Image", command=self.get_file
        )
        update_img_btn.place(anchor="c", relx=0.5, rely=0.35)
        img_display_label = Label(update_products_page, text="Click on image to change")

        product_name_label = Label(update_products_page, text="Product name")
        product_name_label.place(anchor="c", relx=0.5, rely=0.4)
        product_name_var = StringVar()
        product_name_var.set(data.get("name"))
        product_name_entry = Entry(update_products_page, textvariable=product_name_var)
        product_name_entry.place(anchor="c", relx=0.5, rely=0.45)

        product_quantity_label = Label(update_products_page, text="Product quantity")
        product_quantity_label.place(anchor="c", relx=0.5, rely=0.5)
        product_quantity_var = IntVar()
        product_quantity_var.set(data.get("quantity"))
        product_quantity_entry = Entry(
            update_products_page, textvariable=product_quantity_var
        )
        product_quantity_entry.place(anchor="c", relx=0.5, rely=0.55)

        product_price_label = Label(update_products_page, text="Product price")
        product_price_label.place(anchor="c", relx=0.5, rely=0.6)
        product_price_var = DoubleVar()
        product_price_var.set(data.get("price"))
        product_price_entry = Entry(
            update_products_page, textvariable=product_price_var
        )
        product_price_entry.place(anchor="c", relx=0.5, rely=0.65)
        ...
        update_product_btn = Button(
            update_products_page, text="Update Product", command=update_product
        )
    ...
    def get_products(self):
        ...
        get_products_page = Frame(self.app, width="1920", height="1080")
        ...
        scroller = Scrollbar(get_products_page, orient=VERTICAL)
        ...
        if resp["status"] == "Success":
            ...
            for x in resp["message"]:
                ...
                name = Label(entry_frame, text="Name: " + x[1], font="Times 15")
                name.place(anchor="c", relx=mv_x, rely=mv_y + 0.13)
                price = Label(
                    entry_frame, text="Price: GHC " + str(x[2]), font="Times 15"
                )
                price.place(anchor="c", relx=mv_x, rely=mv_y + 0.16)
                quantity = Label(
                    entry_frame, text="Quantity: " + str(x[4]), font="Times 15"
                )
                ...
                edit_btn = Button(
                    ...
                    text="Edit",
                    width=10,
                    command=lambda id=x[0], name=x[1], price=x[2], quantity=x[
                        4
                    ]: self.update_product(
                        {
                            "id": id,
    ...
    def users(self):
        ...
        admin_list = []
        users_page = Frame(self.app, width="1920", height="1080")
        ...
        scroller = Scrollbar(users_page, orient=VERTICAL)
        scroller.place(anchor="c", relx=0.97, rely=0.52, width=20, height=900)
        ...
        if resp["status"] == "Success":
            ...
            mv_y = 0.1
            for x in resp["message"]:
                username = Label(entry_frame, text="Name: " + x[1], font="Times 15")
    ...
    def check_sales(self):
        check_sales_page = Frame(self.app, width="1920", height="1080")
    ...
    def homepage(self):
        admin_page_handler = Frame(self.app, width="1920", height="1080")
        ...
        top_label = Label(
            btns_frame, text="SELECT AN ACTION", fg="blue", font="Times 20 bold"
        )
...
def login(role):
    ...
    login_handler = Frame(supermarket_app, width="1920", height="1080")

    Label(login_handler, text="Enter credentials to proceed").place(
        anchor="c", relx=0.5, rely=0.2
    )

    Label(login_handler, text="Enter Username").place(anchor="c", relx=0.5, rely=0.3)

    username_var = StringVar()
    username_entry = Entry(login_handler, textvariable=username_var, width=20)
    username_entry.place(anchor="c", relx=0.5, rely=0.35)

    password_var = StringVar()
    Label(login_handler, text="Enter Password").place(anchor="c", relx=0.5, rely=0.4)
    password_entry = Entry(login_handler, textvariable=password_var, width=20)
    password_entry.place(anchor="c", relx=0.5, rely=0.45)
    ...
    Button(login_handler, text="LOGIN", command=authenticate).place(
...
cashier_page = CashierPage(supermarket_app)
