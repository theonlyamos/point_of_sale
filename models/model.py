...
class Model():
    ...
    def __init__(self, created_at=None, updated_at=None, id=None):
        self.created_at = (datetime.utcnow()).strftime("%a %b %d %Y %H:%M:%S") \
            if not created_at else created_at.strftime("%a %b %d %Y %H:%M:%S")
        self.updated_at = (datetime.utcnow()).strftime("%a %b %d %Y %H:%M:%S") \
            if not updated_at else updated_at.strftime("%a %b %d %Y %H:%M:%S")
    ...
    @classmethod
    def from_csv(cls, filename: str)-> list[object]:
        '''
        Load Model data from csv_file to database

        @param filename str Name of csvfile to load
        @return List[Models]
        '''
        try:
            import pandas as pd
            ...
            for l in dataframe.values:
                ...
                for i in range(len(dataframe.columns)):
                    d[dataframe.columns[i]] = l[i] if str(l[i]) != 'nan' else ''
                dict_list.append(d)
            print(dict_list)
            return [cls(**elem) for elem in dict_list]

        except ImportError:
            print('Pandas is needed for this function to work')
            # sys.exit(1)
            return []
